...
const nextCourseId = (skill = "GN") =>
  `NLD-${skill.slice(0, 2).toUpperCase()}-${Math.floor(1000 + Math.random() * 9000)}`;
...
const rateLimitCheck = (ip, endpoint) => {
  ...
  const recent = rateLimitLog.filter(
    (e) => e.ip === ip && e.endpoint === endpoint && e.at > cutoff,
  );
...
const rateLimitRecord = (ip, endpoint) => {
  ...
  while (rateLimitLog.length > 0 && rateLimitLog[0].at < cutoff)
    rateLimitLog.shift();
};
...
const courses = [
  {
    ...
    skill: "Security",
    tags: ["OWASP", "Cloud"],
    contentTypes: ["PDF", "Rich Text", "Video Link"],
    approval: "Approved",
    status: "Active",
    ownerId: "EMP-1288",
    version: 3,
  ...
  },
  {
    ...
    description: "Coaching rituals and measurable development plans.",
    skill: "Leadership",
    tags: ["People", "Coaching"],
    contentTypes: ["Rich Text", "Uploaded Video"],
    approval: "Pending",
    status: "Active",
    ownerId: "EMP-1001",
    version: 1,
...
const assessments = [
  {
    id: "ASM-9001",
    title: "Secure Release Readiness",
    courseId: "NLD-SC-2048",
    type: "Timed Quiz",
    approval: "Approved",
    strictMode: true,
    questions: [
      {
        id: "Q1",
        type: "MCQ",
        prompt: "Best control for leaked API key prevention?",
        options: [
          "Manual approval",
          "CI secret scanning",
          "Newsletter",
  ...
  {
    id: "ASM-9038",
    title: "Feedback Scenario Review",
    courseId: "NLD-LD-1130",
    type: "Descriptive",
    approval: "Pending",
    strictMode: true,
    questions: [
      {
        id: "Q1",
        type: "Descriptive",
        prompt: "Write feedback for a missed target using SBI.",
      },
    ],
    durationMinutes: 60,
    passScore: 70,
...
const authenticate = (req, res, next) => {
  ...
  try {
    ...
    if (user.status !== "Active")
      return res
        .status(401)
        .json({ error: "Account deactivated — session revoked" });
    const currentSv = sessionVersions[user.id] || 0;
    if (payload.sv !== undefined && payload.sv !== currentSv) {
      return res
        .status(401)
        .json({ error: "Session revoked — please sign in again" });
    }
    req.user = user;
    return next();
  } catch {
...
const requireRole =
  ...
  (req, res, next) => {
    const effective =
      req.user.role === "Super Admin" && allowed.includes("Admin")
        ? true
...
app.post("/api/auth/login", (req, res) => {
  ...
  const security = loginSecurity[email] || {
    failedAttempts: 0,
    lockedUntil: null,
    lastFailedAt: null,
  };
  if (
    ...
    new Date(security.lockedUntil).getTime() > Date.now()
  ) {
    return res
      .status(403)
      .json({
  ...
  (req, res) => {
    ...
    let visible =
      req.user.role === "Admin" || req.user.role === "Super Admin"
        ? users
        : users.filter(
            (user) => user.managerId === req.user.id || user.id === req.user.id,
          );
    if (department)
      visible = visible.filter((user) => user.department === department);
    if (role) visible = visible.filter((user) => user.role === role);
    if (status !== "All")
      visible = visible.filter((user) => user.status === status);
...
app.patch(
  ...
  (req, res) => {
    ...
    res.json({
      ...
      note: "Session revoked if inactivated.",
    });
  },
);
...
app.delete(
  ...
  (req, res) => {
    ...
    res.json({
      success: true,
      message: "User permanently deleted and archived",
    });
  },
);
...
app.get("/api/courses", authenticate, (req, res) => {
  ...
  if (req.user.role === "Manager")
    visible = visible.filter(
      (c) => c.ownerId === req.user.id || c.approval === "Approved",
    );
...
app.post(
  ...
  (req, res) => {
    ...
    const course = {
      ...
      createdAt: now(),
      updatedAt: now(),
      storage: { files: Object.keys(req.files || {}) },
    };
    courses.unshift(course);
    audit(req.user.id, "CREATE_COURSE", "COURSE", course.id, {
...
app.put(
  ...
  (req, res) => {
    ...
    audit(req.user.id, "UPDATE_COURSE", "COURSE", course.id, {
...
app.post(
  ...
  (req, res) => {
    ...
    audit(req.user.id, "DUPLICATE_COURSE", "COURSE", copy.id, {
      sourceId: source.id,
    });
...
app.delete(
  ...
  (req, res) => {
    ...
    res.json({
      success: true,
      message: "Course permanently deleted and archived",
    });
  },
);
...
app.post(
  ...
  (req, res) => {
    ...
    if (!parsed.success)
      return res.status(400).json({ error: parsed.error.flatten() });
    const created = parsed.data.userIds.map((userId) => ({
      id: uuid(),
    ...
    audit(req.user.id, "ASSIGN_COURSE", "COURSE", parsed.data.courseId, {
...
app.get("/api/assignments", authenticate, (req, res) => {
  ...
  if (req.user.role === "Employee")
    visible = visible.filter((a) => a.userId === req.user.id);
  if (req.user.role === "Manager") {
    const teamIds = new Set(
      users.filter((u) => u.managerId === req.user.id).map((u) => u.id),
    );
    visible = visible.filter(
      (a) => teamIds.has(a.userId) || a.userId === req.user.id,
    );
  }
  res.json({ data: visible, total: visible.length });
});
...
app.get("/api/assessments", authenticate, (req, res) => {
  ...
  if (req.user.role === "Manager")
    visible = visible.filter(
      (item) => item.ownerId === req.user.id || item.approval === "Approved",
    );
  if (req.user.role === "Employee")
    visible = visible.filter((item) => item.approval === "Approved");
  res.json({ data: visible, total: visible.length });
});
...
app.post(
  ...
  (req, res) => {
    ...
    audit(req.user.id, "CREATE_ASSESSMENT", "ASSESSMENT", assessment.id, {
...
app.delete(
  ...
  (req, res) => {
    ...
    res.json({
      success: true,
      message: "Assessment permanently deleted and archived",
    });
  },
);
...
app.post("/api/assessments/:id/attempts/start", authenticate, (req, res) => {
  const assessment = assessments.find(
    (item) => item.id === req.params.id && item.approval === "Approved",
  );
  if (!assessment)
  ...
  res
    ...
    .json({
      data: attempt,
      strictRules: assessment.strictMode ? settings : null,
    });
});
...
app.post(
  ...
  (req, res) => {
    const attempt = attempts.find(
      (item) => item.id === req.params.attemptId && item.userId === req.user.id,
    );
    ...
    const mcqScore = mcqQuestions.length
      ? Math.round(
          (mcqQuestions.filter((q) => req.body.answers?.[q.id] === q.answer)
            .length /
            mcqQuestions.length) *
            100,
        )
    ...
    Object.assign(attempt, {
      submittedAt: now(),
      status: mcqScore === null ? "PendingManualReview" : "Submitted",
      score: mcqScore,
    });
    audit(req.user.id, "SUBMIT_ATTEMPT", "ASSESSMENT_ATTEMPT", attempt.id, {
      score: mcqScore,
    });
    res.json({
      data: attempt,
...
app.post("/api/proctoring/events", authenticate, (req, res) => {
  ...
  if (
    ...
    parsed.data.eventType === "TAB_SWITCH" &&
    attempt
  )
    Object.assign(attempt, { status: "AutoSubmitted", submittedAt: now() });
  audit(req.user.id, "PROCTORING_EVENT", "PROCTORING_LOG", event.id, {
    eventType: event.eventType,
  });
  res
    .status(201)
    .json({ data: event, autoSubmitted: attempt?.status === "AutoSubmitted" });
});
...
app.get(
  ...
  (_req, res) => {
    res.json({
      ...
      approvedCourses: courses.filter((c) => c.approval === "Approved").length,
      completionRate: 78,
      averageScore: 86,
...
app.get("/api/analytics/me", authenticate, (req, res) => {
  res.json({
    ...
    suggestions: [
      "Complete Secure Coding for Cloud Teams",
...
app.post(
  ...
  (req, res) => {
    const schema = z.object({
      ...
      reportType: z.enum(["User", "Team", "Course"]),
    ...
    const report = {
      ...
      status: "Ready",
      downloadUrl: `/exports/${uuid()}.${parsed.data.format === "PDF" ? "pdf" : "xlsx"}`,
      ...parsed.data,
      createdAt: now(),
    };
...
app.get("/api/certificates", authenticate, (req, res) => {
  ...
  if (req.user.role === "Manager") {
    ...
    visible = visible.filter(
      (cert) => cert.employeeId === req.user.id || teamIds.has(cert.employeeId),
    );
...
app.post(
  ...
  (req, res) => {
    ...
    audit(req.user.id, "ISSUE_CERTIFICATE", "CERTIFICATE", cert.id, {
...
app.post(
  ...
  (req, res) => {
    ...
    if (record.entityType === "User") users.unshift(record.entityData);
      ...
      assessments.unshift(record.entityData);
...
app.patch("/api/settings", authenticate, requireRole("Admin"), (req, res) => {
  const schema = z.object({
    ...
    noiseThreshold: z.number().min(1).max(100).optional(),
    maxAssessmentFlags: z.number().min(0).optional(),
  });
  const parsed = schema.safeParse(req.body);
  if (!parsed.success)
    return res.status(400).json({ error: parsed.error.flatten() });
